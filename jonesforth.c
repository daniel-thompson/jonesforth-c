...
/*
	A sometimes minimal FORTH compiler and tutorial for any C compiler that
	implements the GNU C labels-as-values language extension.

	By Daniel Thompson <daniel@redfelineninja.org.uk>
	  and based on the fabulous public domain assembler version for Linux / i386
...
/*
 ...
 <------------------------------------------------------------------------------------------------------------------------>
	...
	If you want to actually run this FORTH, rather than just read it, you will need machine
	with a suitable C compiler.

	There is a simple makefile to support basic builds on most Unix-like systems:
 ...
 *
 * This simply declares the state variables and jumps to
 * start (which is defined at the very bottom of this file)
 *
 * In the middle is a bunch of macro soup that allows us to
 * implement indirect threading by taking the address of the
 * labels (a GNU C extension).
 */
void go_forth(struct forth_task *ctx)
{
...
/*
 * Generate an entry point and dictionary entry for a word implemnted in C.
 *
...
/* Most words don't need any flags to we provide marcos to reduce boilerplate */
#define NATIVE(label, wname) NATIVE_FLAGS(label, wname, 0)
#define BUILTIN(label, wname) BUILTIN_FLAGS(label, wname, 0)

/*
 * Finally, we can reduce the boilerplate even further for words with C friendly spellings
 * (the Q here stands for Quick because there is less typing)
 */
#define QNATIVE(label) NATIVE_FLAGS(label, #label, 0)
#define QBUILTIN(label) BUILTIN_FLAGS(label, #label, 0)

/* Reduce the boilerplate further for normal words with C friendly spellings */
#define SIMPLE_PRIMITIVE(label) PRIMITIVE(#label, label, 0)
...
#define LINK INTERPRET
	...
	if (!var_STATE || (*flags & F_IMMED)) { // executing
		if (is_literal) {
			...
			struct header *hdr = dict_codeword_to_link(codeword);
		...
		struct header *hdr = dict_codeword_to_link(codeword);
		if (is_literal) {
			fprintf(stderr, "COMPILE: %s %" PRIdPTR "\n", (char *) (hdr+1), literal);
		} else {
...
int main(int argc, const char *argv[])
{
	struct forth_task task = {
		.dsp = stalloc(4096 * sizeof(uintptr_t)),
		.rsp = stalloc(4096 * sizeof(uintptr_t)),
		.here = malloc(256*1024),
	};
